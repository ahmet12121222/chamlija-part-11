...
create unique index if not exists payments_booking_provider_unique_idx
  on public.payments (booking_id, provider, provider_payment_id)
  where provider_payment_id is not null;

create index if not exists payments_booking_idx
  on public.payments (booking_id, provider, status);
