...
ALTER TABLE public.bookings
  DROP CONSTRAINT IF EXISTS bookings_selected_area_id_fkey;
...
ALTER TABLE public.bookings
  DROP CONSTRAINT IF EXISTS bookings_selected_paid_activity_id_fkey;
...
ALTER TABLE public.bookings
  DROP CONSTRAINT IF EXISTS bookings_selected_tent_area_id_fkey;
...
ALTER TABLE public.bookings
  DROP CONSTRAINT IF EXISTS bookings_selected_photo_shoot_id_fkey;
...
insert into public.products (name, category, description, price, currency, is_active, is_bookable, is_free, item_order, size, entry_fee_excluded)
values
  ('White Swan & Heart-Shaped Pool', 'picnic_area', 'Bookable picnic area', 2500.00, 'ZAR', true, true, false, 1, null, false),
  ('Picnic - Braai Area', 'picnic_area', 'Bookable braai picnic area', 350.00, 'ZAR', true, true, false, 2, null, false),
  ('Ottoman Corner', 'picnic_area', 'Bookable seating area', 1500.00, 'ZAR', true, true, false, 3, null, false),
  ('6-Seater Picnic Table - Bench', 'equipment', 'Optional equipment', 70.00, 'ZAR', true, true, false, 4, null, false),
  ('Plastic Table', 'equipment', 'Optional equipment', 60.00, 'ZAR', true, true, false, 5, null, false),
  ('Plastic Chair', 'equipment', 'Optional equipment', 20.00, 'ZAR', true, true, false, 6, null, false),
  ('Golf Cart - 4-Seater with Driver', 'paid_activity', 'Paid activity service', 2000.00, 'ZAR', true, true, false, 7, null, false),
  ('Grass Area with Tent', 'tent_event_area', 'Tent area booking', 10000.00, 'ZAR', true, true, false, 8, null, false),
  ('Pangola Tent', 'tent_event_area', 'Tent option', 100.00, 'ZAR', true, true, false, 9, '3×3 m', false),
  ('Grass Area', 'tent_event_area', 'Tent area booking', 5500.00, 'ZAR', true, true, false, 10, null, false),
  ('Frame Tent - 6×9 m', 'tent_event_area', 'Frame tent size option', 2500.00, 'ZAR', true, true, false, 11, '6×9 m', false),
  ('Frame Tent - 5×15 m', 'tent_event_area', 'Frame tent size option', 4000.00, 'ZAR', true, true, false, 12, '5×15 m', false),
  ('Frame Tent - 9×16 m', 'tent_event_area', 'Frame tent size option', 5500.00, 'ZAR', true, true, false, 13, '9×16 m', false),
  ('Photo Shoot - 0–4 hours', 'photo_shoot', 'Photo shoot package', 600.00, 'ZAR', true, true, false, 14, '0–4 hours', true),
  ('Photo Shoot - Full Day', 'photo_shoot', 'Photo shoot package', 1200.00, 'ZAR', true, true, false, 15, 'Full Day', true)
...
create policy if not exists "Public cannot read bookings"
on public.bookings
for select
to public
using (false);

create policy if not exists "Public cannot update bookings"
on public.bookings
for update
to public
using (false)
...
create policy if not exists "Public cannot delete bookings"
on public.bookings
for delete
to public
using (false);
